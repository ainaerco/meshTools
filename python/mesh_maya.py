...
class MayaMesh(Mesh):
	def __init__(self,**kwargs):
		...
		if kwargs.get("bbox",0):
			...
			min = bbox.min()*m
			max = bbox.max()*m
			self.bbox = BBox(min = Vector(min.x,min.y,min.z),max = Vector(max.x,max.y,max.z))
		...
		if kwargs.get("vertices",0):
			...
			for i in xrange(numVertices):
				self.vertices.append(Point(verticesArray[i][0],verticesArray[i][1],verticesArray[i][2]))
	...
	def loadEdges(self):
		...
		for i in xrange(0,numEdges):
			...
			self.edges.append([x.getInt2ArrayItem( y, 0, 0 ),x.getInt2ArrayItem( y, 0, 1 )])
	...
	def selectionToMaya(self,sel,sel_type):
		cmds.select(cl=True)
		if sel_type == "faces" : sel_type = ".f"
		if sel_type == "vertices" : sel_type = ".vtx"
		if sel_type == "edges" : sel_type = ".e"
