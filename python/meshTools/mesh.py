import random,math#degrees,tan,pi,sin,cos,modf
...
#import triangulate
#reload(triangulate)
# from triangulate import Polygon
...
class Mesh(object):
    ...
    def __str__(self):
        ...
        for i in self.normals:
            n = n+","+str(i)
        return "face count: "+str(len(self.faces))+"\nvertex count: "+str(len(self.vertices))+"\nfaces: "+str(self.faces)+"\nvertices: "+s+"\nnormals: "+n
    ...
    def findVertexNeighbor(self,vertex,faces):
        ...
        for face in faces:
            ...
            if inface_id>-1:
                ...
                if find ==-1:neighbors.append(first)
                find = lists.find(neighbors,last)
                if find ==-1:neighbors.append(last)
        return neighbors
    ...
    def __selectFaceBorders(self,select):
        ...
        #separate by pairs
        for f in select:
            for i in xrange(1,len(self.faces[f])):
                border_edges.append([self.faces[f][i-1],self.faces[f][i]])
            border_edges.append([self.faces[f][len(self.faces[f])-1],self.faces[f][0]])
        #print border_edges

        #remove duplicates
        border = []
        for i in xrange(0,len(border_edges)):
            if not(len(lists.duplicates_of(border_edges, border_edges[i]))==1 and len(lists.duplicates_of(border_edges, [border_edges[i][1],border_edges[i][0]]))==1):
    ...
    def __selectFaceBordersEx(self,select):
        ...
        border_group_dup=[]
        for i in xrange(0,len(border_vertices)):
            border_group_dup.append([])
            for k in xrange(0,len(border_vertices[i])):
                for j in select:
        ...
        border_group=[]
        for i in xrange(0,len(border_group_dup)):
    ...
    def __selectFacesInterior(self,select):
        all_verts = []
        for f in select:
            for i in xrange(len(self.faces[f])):
    ...
    def __selectVertexGroups(self,verts):
        ...
        cverts = cverts[1:]
        while len(cverts)>0:
            neighbors = self.__selectConvertFV(self.vertices[cverts[0]].parent_faces)
            count = 0
            finds = []
            for neighbor in neighbors:
                if neighbor in passed_verts:
                    for i in xrange(len(vert_groups)):
                        if (neighbor in vert_groups[i]) and\
                        not cverts[0] in vert_groups[i]:
            ...
            if count>1:
                ...
                for i in xrange(len(finds)-1):
                    vert_groups_combine+=vert_groups[finds[i]+1:finds[i+1]]
                vert_groups_combine+=vert_groups[finds[-1]+1:]
                #print "vert_groups_combine",vert_groups_combine
                vert_groups_combine+=[vert_groups[finds[0]]]
                #print "vert_groups_combine",vert_groups_combine
                for i in xrange(1,len(finds)):
    ...
    def __selectConvertVF2(self,edges):
        ...
        for i in xrange(0,len(self.faces)):
            ...
            for j in xrange(0,len(self.faces[i])):
                if j==len(self.faces[i])-1:
                    ...
                    mesh_edges[i].extend([[self.faces[i][j],self.faces[i][j+1]]])
                    mesh_edges[i].extend([[self.faces[i][j+1],self.faces[i][j]]])
        sel_faces=[]
        #print mesh_edges
        for i in xrange(0,len(self.faces)):
            for edge in edges:
    ...
    def __selectConvertFEV(self,faces):
        ...
        for face in faces:
            ...
            for i in xrange(len(face_verts)):
                if i==len(face_verts)-1:edge_verts.append([face_verts[i],face_verts[0]])
                else:edge_verts.append([face_verts[i],face_verts[i+1]])
        return edge_verts
    ...
    def __selectConvertFE(self,faces):
        ...
        edges=[]
        for i in xrange(len(edge_verts)):
    ...
    def selectGrowV(self,sel_verts):
        ...
        for vert in sel_verts:
            new_verts+=[self.findVertexNeighbor(vert,self.vertices[vert].parent_faces)]
    ...
    def __selectLoopGroupAnchors(self,border):
        ...
        for i in xrange(len(border)):
            ...
            else: anchor_faces_old = self.__selectConvertVF([border[i]])
            anchor_faces = self.__selectConvertVF([neighbors[0]])
            #print "anchor_faces",anchor_faces
            #print "anchor_faces_old",anchor_faces_old
            anchor_similar_face=lists.leave_duplicates(anchor_faces,anchor_faces_old)
    ...
    def __selectConvertVE(self,sel_verts):
        ...
        for i in xrange(len(self.edges)):
            if self.edges[i][0] in sel_verts:sel_edges+=[i]
            elif self.edges[i][1] in sel_verts:sel_edges+=[i]
        return sel_edges

    def selectConvert(self,selection,fromtype,totype):
        if fromtype==kGeotype.face:
            if totype==kGeotype.edge:return self.__selectConvertFE(selection)
            elif totype==kGeotype.vertex:return self.__selectConvertFV(selection)
        if fromtype==kGeotype.edge:
            if totype==kGeotype.face:return self.__selectConvertEF1(selection)
            elif totype==kGeotype.vertex:return self.__selectConvertEV(selection)
        if fromtype==kGeotype.vertex:
            if totype==kGeotype.face:return self.__selectConvertVF(selection)
            elif totype==kGeotype.edge:return self.__selectConvertVE(selection)
    ...
    def selectSimilarF(self,sel_face):
        ...
        for i in xrange(1,len(self.faces)):
        ...
        if find>-1:
            return sorted_faces[faces_groups[find][0]:faces_groups[find][1]+1]
        else: return 0


    def selectFByAngle(self,sel_faces,angle=.0):
        grow_faces=copy(sel_faces)
        for face in grow_faces:
            grows=self.selectGrowF([face])
            for grow in grows:
                if not grow in grow_faces and math.degrees(self.getNormal(face).angle(self.getNormal(grow)))<=angle:
                    grow_faces += [grow]
        return grow_faces
        ...
        def sort_normals(a,b):
            if a[0].x<b[0].x:return -1
                ...
                if a[0].y<b[0].y:return -1
                    ...
                    elif a[0].z>b[0].z:return 1
                    else:return 0
        def find_normal(sorted_normals,normal,delta):
            start = -1
            end = -1
            
            for i  in xrange(1,len(sorted_normals)):
                if start ==-1:

                    if sorted_normals[i-1][0].x-delta<=normal.x and normal.x<=sorted_normals[i][0].x+delta:
                        if sorted_normals[i-1][0].y-delta<=normal.y and normal.y<=sorted_normals[i][0].y+delta:
                            if sorted_normals[i-1][0].z-delta<=normal.z and normal.z<=sorted_normals[i][0].z+delta:
                                start = i-1
                elif sorted_normals[i][0].x+delta<normal.x or sorted_normals[i][0].y+delta<normal.y or sorted_normals[i][0].z+delta<normal.z  :
                    end = i
                    break
            return [start,end]
        new_sel_faces=[]
    ...
    def __spherifyPoint(self,center,radius,vert,factor):
        vert1 = vert-center
        vert1.normalize()
        t=radius/((vert1.x*vert1.x+vert1.y*vert1.y+vert1.z*vert1.z)**0.5)
    ...
    def __computeFaceNormal(self,face):
        ...
        for i in xrange(len(self.faces[face])):
            ...
            if i==len(self.faces[face])-1: v2=self.vertices[self.faces[face][0]]
            else: v2=self.vertices[self.faces[face][i+1]]
            normal = normal + v1.cross(v2)
        normal = normal.normalize()
        return normal

    def __computeVertexNormal(self,vert,faces=None):
        if not faces: neighbors = self.findVertexNeighbor(vert,self.vertices[vert].parent_faces)
        else: neighbors = self.findVertexNeighbor(vert,faces)
        normal = Vector()
        for i in xrange(len(neighbors)-1):
            normal += (self.vertices[neighbors[i]]-self.vertices[vert]).cross(self.vertices[neighbors[i+1]]-self.vertices[vert])
        normal = normal.normalize()
        return normal

    def __faceCenter(self,face):
        center = Point()
        for i in xrange(0,len(self.faces[face])):
    ...
    def __sortVerticesCCW(self,sel_verts):
        ...
        for i in xrange(0,len(sel_verts)):
        ...
        def clockwise_sort(a,b):
            if a[1].x>=0 and b[1].x<0: return -1
            if a[1].x==0.0 and b[1].x==0: return a[1].z<=b[1].z
    ...
    def updateFace(self,id,value):
        ...
        for vert in self.faces[id]:
            self.vertices[vert].parent_faces = lists.remove_valuez(self.vertices[vert].parent_faces,id)
    ...
    def __updateFaceVert(self,id,find_id,repl_id):
        self.updateFace(id,self.faces[id][:find_id]+[repl_id]+self.faces[id][find_id+1:])
    ...
    def __connectFace(self,sel_face,first,second,connections):
        ...
        for i in xrange(1,connections):
            first_con=first+i
            second_con=second-i
            #print face_cycle[first_con],face_cycle[first_con+1],face_cycle[second_con-1],face_cycle[second_con]
            new_faces.append([face_cycle[second_con],face_cycle[second_con+1],face_cycle[first_con-1],face_cycle[first_con]])
        new_faces += [(face_cycle[first+connections-1:second-connections+2]).list]
    ...
    def test(self):
        ...
        for v in xrange(len(self.vertices)):
    ...
    def multiDuplicateTransform(self,mesh,matrix,**kwargs):
        ...
        if do_faces:
            ...
            for i in xrange(len(mesh.faces)):
                new_faces += [[]]
                for j in xrange(len(mesh.faces[i])):
    ...
    def gridTasselate(self,selection,**kwargs):
        ...
        if selection_type != kGeotype.face:
            selection = self.selectConvert(selection,selection_type,kGeotype.face)

        #self.bbox = [Point(min.x,min.y,min.z),Point(max.x,max.y,max.z),Point(center.x,center.y,center.z)]
        if self.bbox==None: return
        ...
        for i in xrange(xsteps):
            self.clipPlane(False,Ray(Vector(self.bbox[0].x+step_size*i,0,0),Vector(1,0,0)))
        for i in xrange(ysteps):
            self.clipPlane(False,Ray(Vector(0,self.bbox[0].y+step_size*i,0),Vector(0,1,0)))
        for i in xrange(zsteps):
            self.clipPlane(False,Ray(Vector(0,0,self.bbox[0].z+step_size*i),Vector(0,0,1)))
    ...
    def delaunay(self):
        ...
        for i in xrange(20):
            ...
            verts+=[Point(fit(random.random(),0,1,min,max),fit(random.random(),0,1,min,max),fit(random.random(),0,1,min,max))]
        verts+=[Point()]
    ...
    def detach(self,selection,**kwargs):
        ...
        for i in xrange(len(new_faces)):
            for j in xrange(len(new_faces[i])):
                new_faces[i][j] = lists.find(old_verts,new_faces[i][j])
        for i in xrange(len(old_verts)):
    ...
    def radialSymmetry(self,copies):
        ...
        self.rebuildVertP()
        self.clipPlane(True,Ray(Vector(0,0,0),Vector(0,1,1/math.tan(angle))))
        ...
        for i in xrange(copies-1):
            ...
            old_vcount = len(self.vertices)
            for i in xrange(len(sym_vertices)):
    ...
    def clipPlane(self,delete_remains = True, ray = Ray(Vector(0,0,0),Vector(0,1,0))):
        verts_rejected = []
        verts_sides = []
        for i in xrange(len(self.vertices)):
            ray_test = ray.pointPlaneSide(self.vertices[i])
            verts_sides.append(ray_test)
            if not ray_test:verts_rejected+=[i]
        ...
        for j in xrange(len(faces_involved)):
            cycle_face = CycleList(self.faces[faces_involved[j]])
            faces_points.append([])
            #computed_edges.append([])
            for i in xrange(len(cycle_face)):
                if verts_sides[cycle_face[i]]+verts_sides[cycle_face[i+1]]==1:
                    find = lists.find(computed_edges,[cycle_face[i], cycle_face[i+1]])
                    if find ==-1:
                        we = ray.segmentPlaneHit([self.vertices[cycle_face[i]],self.vertices[cycle_face[i+1]]])
                        if we!=0:
                            #computed_edges[j].extend([[cycle_face[i], cycle_face[i+1]],[cycle_face[i+1], cycle_face[i]]])
                            computed_edges+=[[cycle_face[i], cycle_face[i+1]],[cycle_face[i+1], cycle_face[i]]]
                            self.addVertex(Point(we.x,we.y,we.z))
                            faces_points[j].extend([len(self.vertices)-1])
                            computed_points+=[len(self.vertices)-1]
                    else:
                        faces_points[len(faces_points)-1].extend([computed_points[int(find/2)]])
        ...
        start_time = time()
        for facenum in xrange(len(faces_involved)):
            ...
            if len(faces_points[facenum])>2:
                ...
                for j in xrange(2,len(faces_points[facenum])):
                    z = self.vertices[faces_points[facenum][j]]
                    xz = (x-z).length()
                    yz = (y-z).length()
                    if xz>yz or yz<xy:sorted_verts+=[[faces_points[facenum][j],xz]]
                    else:sorted_verts+=[[faces_points[facenum][j],-xz]]
            ...
            for j in xrange(len(edges)):
                if verts_sides[edges[j][0]]==False and verts_sides[edges[j][1]]==False:
                    edges[j]=[]
                else:
                    find = lists.find(computed_edges,edges[j])/2
                    if find>-1:
                        findy = lists.find(sorted_verts,computed_points[find])/2
                        if verts_sides[edges[j][0]]:edges[j] = [edges[j][0]]+pair_sorted_verts[findy]
                        else:edges[j] = pair_sorted_verts[findy]+[edges[j][1]]
            if not delete_remains:
                for j in xrange(len(edges_bottom)):
                    if verts_sides[edges_bottom[j][0]]==True and verts_sides[edges_bottom[j][1]]==True:
                        edges_bottom[j]=[]
                    else:
                        find = lists.find(computed_edges,edges_bottom[j])/2
                        if find>-1:
                            findy = lists.find(sorted_verts,computed_points[find])/2
                            if not verts_sides[edges_bottom[j][0]]:edges_bottom[j] = [edges_bottom[j][0]]+pair_sorted_verts[findy][::-1]
                            else:edges_bottom[j] = pair_sorted_verts[findy][::-1]+[edges_bottom[j][1]]
    ...
    def triangulate(self,selection,**kwargs):
        ...
        for face in selection:
            ...
            poly = Polygon(self.vertices,self.faces[face],self.getNormal(face))
            triPlainIndices = poly.triangulate()
            triIndices = []
            for i in xrange(len(triPlainIndices)/3):
                triIndices += [[triPlainIndices[i*3],
    ...
    def makeNgon(self,selection,**kwargs):
        ...
        if selection_type != kGeotype.vertex:
            selection = self.selectConvert(selection,selection_type,kGeotype.vertex)
        ...
        for group in groups:
            ...
            for i in xrange(sides):
                p = Point(math.sin(2*math.pi/sides*(i+cycle+1))*radius,math.cos(2*math.pi/sides*(i+cycle+1))*radius,0)
    ...
    def makePlanar(self,selection,**kwargs):
        ...
        for vert in verts:
            self.updateVertex(vert,plane.segmentPlaneHit([self.vertices[vert],self.vertices[vert]+avg_normal]))
        return [kResult.updateVertex]
    ...
    def dissolve(self,selection,**kwargs):
        ...
        border_verts=[]
        for i in xrange(len(border_faces)):
    ...
    def noise(self,selection,**kwargs):
        ...
        for vert in selection:
            ...
            #self.pushVertices([vert],v*amount)
            v = n.turbulence(self.vertices[vert].x,self.vertices[vert].y,self.vertices[vert].z,2,1.2,amount)
            #v = n.fBm(self.vertices[vert].x,self.vertices[vert].y,self.vertices[vert].z,8.2,1.2,amount)
            self.updateVertex(vert,Point(self.vertices[vert].x,self.vertices[vert].y+v,self.vertices[vert].z))
        start_time = time()-start_time
    ...
    def push(self,selection,**kwargs):
        ...
        for vert in selection:
            ...
            normal = normal.normalize()
            for i in xrange(len(faces_involved)):
                angle = max(angle,normal.angle(faces_normal[i]))
            if angle<EPSILON or EPSILON<angle:
                self.vertices[vert].addRuler(normal*push_amount/math.cos(angle))
            else:
                self.vertices[vert].addRuler(normal*push_amount)
        for vert in selection:
            self.updateVertex(vert,self.vertices[vert]+self.vertices[vert].ruler)
        return [kResult.updateVertex]
    ...
    def relax(self,selection,**kwargs):
        ...
        for iteration in xrange(iterations):
            for v in xrange(len(selection)):
                newPos = Point()
                for i in neighbor_list[v]:
                    newPos += self.vertices[i]
                newPos = newPos/len(neighbor_list[v])
                self.updateVertex(selection[v],self.vertices[selection[v]].lerp(newPos,factor))
                #Additive smoothing
    ...
    def spherify(self,selection,**kwargs):
        ...
        for sel_vert in selection:
            self.updateVertex(sel_vert,self.__spherifyPoint(center,radius,self.vertices[sel_vert],factor))
        return [kResult.updateVertex]
    ...
    def collapse(self,selection,**kwargs):
        ...
        if selection_type != kGeotype.vertex:
            selection = self.selectConvert(selection,selection_type,kGeotype.vertex)
        if group:groups = self.__selectVertexGroups(selection)
        else: groups = [selection]
        delete_faces = []
        delete_verts = []
        ...
        for vert in selection:
            ...
            for face in faces:
                ...
                for face_vert in face_verts:
                    ...
                    if connect_vert!=-1:
                        neighbors = self.findVertexNeighbor(selection[connect_vert],[face])
                        if not vert in neighbors:
                            id1 = lists.find(self.faces[face],vert)
                            id2 = lists.find(self.faces[face],selection[connect_vert])
                            if id1>id2: id1,id2=id2,id1
                            face1 = self.faces[face][id2:]+self.faces[face][0:id1+1]
    ...
    def recursiveFillCorner(self,mid,corner,outer_points,divisions,start_divisions,center,new_faces,depth=1):
        inner_points=[]
        corner_sides = len(corner)
        half_start_divisions = math.modf(start_divisions/2.0)[1]+2*math.modf(start_divisions/2.0)[0]
        half_divisions = math.modf(divisions/2.0)[1]+2*math.modf(divisions/2.0)[0]
        is_even = 2*math.modf((divisions+1)/2.0)[0]

        #fill inner points
        for k in xrange(0,corner_sides):
            if (k+1)*divisions>len(outer_points)-1: a = self.vertices[outer_points[0]]
            else: a = self.vertices[outer_points[(k+1)*divisions]]
            if k == 0: b = self.vertices[outer_points[len(outer_points)-1]]
            else: b = self.vertices[outer_points[k*divisions-1]]

            if is_even==0:#if no even
                median = (mid[k]).lerp(center,depth/float(half_start_divisions))
                for i in xrange(0,half_divisions-2):
                ...
                for i in xrange(0,half_divisions-1):
                ...
                median = (mid[k]).lerp(center,depth*2*is_even/float(start_divisions+is_even))
                for i in xrange(0,half_divisions):
                    j=half_divisions-1-i
                    p = median.lerp(b,2/float(divisions+1)*is_even*j+1/float(divisions+1)*is_even)
                    self.vertices.append(p)
                    inner_points.extend([len(self.vertices)-1])
                ...
                for i in xrange(0,half_divisions-1):
                    p = median.lerp(a,2/float(divisions+1)*is_even*i+1/float(divisions+1)*is_even)
                    self.vertices.append(p)
                    inner_points.extend([len(self.vertices)-1])
        ...
        new_corner = []
        if is_even==0:
            for i in xrange(0,corner_sides):
                we = int((i+1)*(divisions-1)-1)
                new_corner.append(inner_points[we])
            new_corner = new_corner[len(new_corner)-1:]+new_corner[0:len(new_corner)-1]
        else:
            for i in xrange(0,corner_sides):
        ...
        k=0
        for i in xrange(0,corner_sides):
            for j in xrange(0,divisions-1):
                #print i*divisions+j,i*divisions+j+1,
                if is_even==0:
                    if k-1>=0:new_faces.append([outer_points[i*divisions+j],outer_points[i*divisions+j+1],inner_points[k],inner_points[k-1]])
                    else:new_faces.append([outer_points[i*divisions+j],outer_points[i*divisions+j+1],inner_points[k],inner_points[len(inner_points)-1]])
                else:
                    if k+1<len(inner_points):new_faces.append([outer_points[i*divisions+j],outer_points[i*divisions+j+1],inner_points[k+1],inner_points[k]])
                    else:new_faces.append([outer_points[i*divisions+j],outer_points[i*divisions+j+1],inner_points[0],inner_points[k]])
                k+=1
            #fill corner faces
            if i*divisions-1>=0:new_faces.append([corner[i],outer_points[i*divisions],new_corner[i],outer_points[i*divisions-1]])
            else:new_faces.append([corner[i],outer_points[i*divisions],new_corner[i],outer_points[len(outer_points)-1]])
        
        if divisions-2>1:
            self.recursiveFillCorner(mid,new_corner,lists.remove_duplicates(inner_points,new_corner),divisions-2,start_divisions,center,new_faces,depth+1)
        elif divisions-2==1:
            self.vertices.append(center)
            mids = lists.remove_duplicates(inner_points,new_corner)
            for i in xrange(0,corner_sides):
                if i == corner_sides-1:new_faces.append([mids[i],new_corner[0],mids[0],len(self.vertices)-1])
                else:new_faces.append([mids[i],new_corner[i+1],mids[i+1],len(self.vertices)-1])
        elif divisions-2==0:
    ...
    def quadChamfer(self,sel_face):
        ...
        self.recursiveFillCorner(mid,corner,outer_points,divisions,divisions,center,new_faces)
    ...
    def quadChamfer1(self,sel_edges,chamfer_size):
        ...
        faces_terminate = lists.remove_duplicates(faces_terminate,faces_involved)
        ...
        for face in faces_involved:
            ...
            for vertex in vertices_involved:
                ...
                if len(neighbors)>0:
                    ...
                    neighbors = lists.remove_duplicates(neighbors,vertices_involved)

                    if len(neighbors)>0:
                        #termination vertex
                        edge_vertex = lists.remove_duplicates(edge_vertex,neighbors)
                        neighbor = neighbors[0]
                        edge_vertex = edge_vertex[0]
                        v2 = Vector()
                        v2.fromPoints(self.vertices[neighbor],self.vertices[vertex])
                        v2 = v2.normalize()
                        ...
                        v1 = Vector()
                        v1.fromPoints(self.vertices[vertex],self.vertices[edge_vertex[0]])
                        v1 = v1.normalize()

                        v2 = Vector()
                        v2.fromPoints(self.vertices[edge_vertex[1]],self.vertices[vertex])
                        v2 = v2.normalize()
                        angle = v1.angle(v2)
                        a=v1-v2
                        if angle<0.1:
        ...
        #for face in faces_involved:
            #for i in xrange(0,len(vertices_involved)):
        ...
        for chamfer_element in chamfer_data:
            if len(chamfer_element)>3:
                ...
                for face_t in faces_terminate:
                    ...
                    if repl_id!=-1:
                        print(repl_id)
                        print(self.faces[face_t],self.faces[face_t][:repl_id-1]+chamfer_element[1:]+self.faces[face_t][repl_id+1:])

                        self.faces[face_t] = self.faces[face_t][:repl_id-1]+chamfer_element[1:]+self.faces[face_t][repl_id+1:]
    ...
    def insertCylinder(self,selection,**kwargs):
        ...
        for i in xrange(len(corner)):
            if i < len(corner)-1:mid = (self.vertices[corner[i]]).lerp(self.vertices[corner[i+1]],0.5)
            else:mid = (self.vertices[corner[i]]).lerp(self.vertices[corner[0]],0.5)
            b = avg_center-mid
            if i == 0: 
                radius = b.length()
                a = b
            ...
            if (a.normalize()).dot(Vector(0,1,0))<(b.normalize()).dot(Vector(0,1,0)):
                a = b

        t = Transform()
        t.lookAt(avg_center+avg_normal.normalize()*1.2,avg_center+avg_normal,a)
        ...
        cycle_rect.cycle(3)
        for i in xrange(sides):
            p = Point(math.sin(2*math.pi/sides*i)*radius,math.cos(2*math.pi/sides*i)*radius,0)
            ...
            if modr[1]==0:   rectp = Point(-.5*radius,(modr[0]-.5)*radius,0)
            elif modr[1]==1: rectp = Point((modr[0]-.5)*radius,0.5*radius,0)
            elif modr[1]==2: rectp = Point(.5*radius,(.5-modr[0])*radius,0)
            elif modr[1]==3: rectp = Point((.5-modr[0])*radius,-.5*radius,0)
            ...
            hitted = False
            k=0
            while k < len(selection) and hitted==False:
        ...
        for i in xrange(0,sides):
            for j in xrange(0,sides):
                new_dist=(self.vertices[new_vertices[i]]-self.vertices[border[j]]).length()
                if i==0 and j==0: min_dist=new_dist
        ...
        new_faces=[]
        for i in xrange(0,sides):
            if i<sides-1:new_faces.append([border[i],border[i+1],new_vertices[i+1],new_vertices[i]])
            else:new_faces.append([border[i],border[0],new_vertices[0],new_vertices[i]])
    ...
    def capParallel(self,selection,**kwargs):
        ...
        for face_id in selection:
            ...
            for i in xrange(0,len(face)-1):
                v1 = Vector()
                v1.fromPoints(self.vertices[face[i]],self.vertices[face[i+1]])
                length = v1.length()
                if len_max < length:
                    len_max = length
                    max_id = i

            self.updateFace(face_id,lists.cycle(self.faces[face_id],max_id+2))
            for i in xrange(0,math.modf(len(face)/2.0)[1]-1):
                #print "connect pairs"
                
                if i==0:
                    self.addFace([face[i],face[len(face)-3-i],face[len(face)-3-i+1],face[len(face)-1]])
                    #print (face[i],face[len(face)-3-i],face[len(face)-3-i+1],face[len(face)-1])
                    #print i,len(face)-3-i,len(face)-3-i+1,len(face)-1
                else:
                    self.addFace([face[i],face[len(face)-3-i],face[len(face)-3-i+1],face[i-1]])
    ...
    def extrude(self,selection,**kwargs):
        ...
        for f in xrange(0,len(borders[0])):
            borders[3].append([])
            #add vertices
            ...
            for j in borders[1][f]:
                #print self.faces[j]
                for k in xrange(0,len(self.faces[j])):
            ...
            for i in range(1,len(borders[0][f])):
                self.addFace([borders[0][f][i-1],borders[0][f][i],borders[3][f][i],borders[3][f][i-1]])

            #update vertex rulers

            borders_rejected = []
            for i in xrange(0,len(borders[2])):
                borders_rejected.append(borders[2][i][0])

            for i in xrange(1,len(borders[0][f])):

                v1 = Vector()
                v2 = Vector()
                if i< len(borders[0][f])-1:
                    v1.fromPoints(self.vertices[borders[3][f][i]],self.vertices[borders[3][f][i-1]])
                    v2.fromPoints(self.vertices[borders[3][f][i+1]],self.vertices[borders[3][f][i]])
                else:
                    v1.fromPoints(self.vertices[borders[3][f][i]],self.vertices[borders[3][f][i-1]])
                    v2.fromPoints(self.vertices[borders[3][f][1]],self.vertices[borders[3][f][i]])
                v1 = v1.normalize()
                v2 = v2.normalize()
                ...
                if len(dup)>=1:
                    c = []
                    for j in dup:
                        c.append((self.vertices[borders[2][j][1]]-self.vertices[borders[2][j][0]] ).normalize())
                    b = Vector()
                    angleb = 0
            ...
            for i in xrange(1,len(borders[3][f])):
                self.updateVertex(borders[3][f][i],self.vertices[borders[3][f][i]]+self.vertices[borders[3][f][i]].ruler)
    ...
    def capCylinder(self,faces):
        ...
        for face in faces:
            ...
            for i in range(0,more_sides):
                ...
                more_connections.append([self.faces[face][i],self.faces[face][c]])
            #add points
            for i in more_connections:
                for j in range(1,less_sides+1):
                    a = self.vertices[i[0]].lerp(self.vertices[i[1]],j/float(less_sides+1))
                    self.addVertex(Point(a.x,a.y,a.z))

            #connect faces
            print("left")
            for i in range(1,more_sides):
                w= [self.faces[face][i],less_sides*i+verts_count_old,less_sides*(i-1)+verts_count_old,self.faces[face][i-1]]
                print(w)
                self.addFace(w)
            print("top")
            for i in range(more_sides+2,more_sides+1+less_sides):
                w = [self.faces[face][i],more_sides*less_sides-less_sides-more_sides-1+i+verts_count_old,more_sides*less_sides-less_sides-more_sides-2+i+verts_count_old,self.faces[face][i-1]]
                print(w)
                self.addFace(w)
            print("right")
            for i in range(more_sides+less_sides+3,more_sides+more_sides+less_sides+2):
                w = [self.faces[face][i],2*less_sides*more_sides-(i-less_sides-2)*less_sides-1+verts_count_old,less_sides*(2*more_sides-i+less_sides+3)-1+verts_count_old,self.faces[face][i-1]]
                print(w)
                self.addFace(w)
            print("bottom")
            for i in range(more_sides*2+less_sides+4,more_sides*2+less_sides+3+less_sides):
                w = [self.faces[face][i],less_sides*2+more_sides*2+2-i+verts_count_old,less_sides*2+more_sides*2+3-i+verts_count_old,self.faces[face][i-1]]
                print(w)
                self.addFace(w)
            print("grid")
            for i in range(1,more_sides):
                for j in range(1,less_sides):
                    w = [j+(i-1)*less_sides-1+verts_count_old,less_sides*i+j-1+verts_count_old,less_sides*i+j+verts_count_old,j+(i-1)*less_sides+verts_count_old]
                    print(w)
                    self.addFace(w)
            print("corners")
            w = [self.faces[face][more_sides-1],self.faces[face][more_sides],self.faces[face][more_sides+1],more_sides*less_sides-more_sides+(more_sides+1)%(less_sides+1)+verts_count_old]
            print(w)
            self.addFace(w)
            w = [self.faces[face][more_sides+less_sides],self.faces[face][more_sides+less_sides+1],self.faces[face][more_sides+less_sides+2],more_sides*less_sides-1+verts_count_old]
            print(w)
            self.addFace(w)
            w = [self.faces[face][2*more_sides+less_sides+1],self.faces[face][2*more_sides+less_sides+2],self.faces[face][2*more_sides+less_sides+3],less_sides-1+verts_count_old]
            print(w)
            self.addFace(w)
            if math.modf(sides/2.0)[0]!=0:
                w = [self.faces[face][2*more_sides+2*less_sides+2],self.faces[face][2*more_sides+2*less_sides+3],verts_count_old]
                print(w)
                self.addFace(w)
                print("not even")
                w = [self.faces[face][2*more_sides+2*less_sides+3],self.faces[face][2*more_sides+2*less_sides+4],self.faces[face][0],verts_count_old]
                print(w)
                self.addFace(w)
            else: 
                w = [self.faces[face][2*more_sides+2*less_sides+2],self.faces[face][2*more_sides+2*less_sides+3],self.faces[face][0],verts_count_old]
                print(w)
                self.addFace(w)

            self.__deleteFaces([face])
            grid_vertices = []
            for i in range(len(self.vertices)-less_sides*more_sides,len(self.vertices)):
    ...
    def straightLoop(self,sel_edges):
        ...
        for border in borders:
            ...
            center = self.vertices[border[0]].lerp(self.vertices[border[-1]],0.5)
            v = v.normalize()
            axis = Vector(0,1,0)
            ...
            t1= t1*t2
            for i in xrange(len(border)):
                self.updateVertex(border[i],t1.applyTransform(self.vertices[border[i]]))
        return [kResult.updateVertex]
    ...
    def flowLoop(self,sel_edges,factor=1):
        ...
        for border in borders:
            ...
            anchors1 = lists.remove_duplicates(list_1dimension(anchors1),border)
            anchors2 = lists.remove_duplicates(list_1dimension(anchors2),border)
            
            for i in xrange(len(border)):
                #print anchors1[i],anchors[i][0],anchors[i][1],anchors2[i]
                v1 = self.vertices[anchors1[i]]
                v2 = self.vertices[anchors[i][0]]
                v3 = self.vertices[border[i]]
                ...
                self.updateVertex(border[i],self.vertices[border[i]].lerp(p,factor))
                #for j in xrange(11):
    ...
    def slideLoop(self,sel_edges,percent=0.5):
        ...
        for border in borders:
            ...
            for i in xrange(len(border)):
                if percent>0:
                    self.updateVertex(border[i],self.vertices[border[i]].lerp(self.vertices[anchors[i][0]],percent))
                else:self.updateVertex(border[i],self.vertices[border[i]].lerp(self.vertices[anchors[i][1]],-percent))
        return [kResult.updateVertex]
    ...
    def spaceLoops(self,sel_edges):
        ...
        for border in borders:
            avg_len = 0
            for i in xrange(1,len(border)):
                avg_len+= (self.vertices[border[i-1]]-self.vertices[border[i]]).length()
            avg_len = avg_len/(len(border)-1)
            for i in xrange(1,len(border)-1):
                edge_v=(self.vertices[border[i]]-self.vertices[border[i-1]]).setLength(avg_len/2.0)
                edge_center=self.vertices[border[i]].lerp(self.vertices[border[i-1]],0.5)
            ...
            for i in xrange(len(border)):
                ...
                if alpha>EPSILON or alpha<-EPSILON:
                    ...
                    if ellipse_interpolation:
                        height = (y*y/(1-x*x/(a-b).length()/(a-b).length()))**0.5
                    else: height = y/2
                else: 
                    x = (a-c).length()/(a-b).length()
                    y = 0
                    height = 0
                self.updateVertex(border[i],a.lerp(b,0.5)-((a-c)-(a-b).setLength(x)).setLength(height))
        return [kResult.updateVertex]
    ...
    def connectEdges(self,sel_edges,connections=1):
        ...
        for i in xrange(len(sel_edges)):
            new_edges.append([])
            for j in xrange(0,connections):
                v1=self.edges[sel_edges[i]][0]
                v2=self.edges[sel_edges[i]][1]
                center = self.vertices[v1].lerp(self.vertices[v2],1/float(connections+1)*(j+1))
                self.addVertex(center)
        ...
        for i in xrange(len(sel_edges)):
            ...
            for face in faces:
                if not face in passed_faces:
                    edges = self.__selectConvertFE([face])
                    for edge in edges:
                        self.updateFace(face,self.faces[face]+new_edges[edge])
                    self.updateFace(face,self.__sortVerticesCCW(self.faces[face]))
                    passed_faces+=[face]
                    ...
                    if len(edges)==2:
                        ...
                        first_last = face_cycle.find(new_edges[edges[0]][connections-1])
                        if first_last>connections+1: 
                            first=first_last
                            face_cycle.cycle(first)
                        self.updateFace(face,face_cycle.list)
                        first = 0
                        second = max(face_cycle.find(new_edges[edges[1]][0]),face_cycle.find(new_edges[edges[1]][connections-1]))
                        new_faces.extend(self.__connectFace(face,first,second,connections))
                        delete_faces+=[face]
