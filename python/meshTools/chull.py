...
class Edge:
    ...
    def __str__(self):
        ...
        return "edge(%d): %s %s del: %s newf: %d adjf:%s" % (
            ...
            self.endpts[1],
            self.delete,
            self.newface.fnum if self.newface is not None else -1,
            " ".join(af),
        )
...
class Face:
    ...
    def __init__(
        self, edge=[None, None, None], vertex=[None, None, None], visible=False
    ):
        self.edge = []
...
vertex %s
endloop
endfacet""" % (self.vertex[0].vnum, self.vertex[1].vnum, self.vertex[2].vnum)
...
class Hull:
    ...
    def EdgeOrderOnFaces(self):
        ...
        for f in self.faces:
            for i in (0, 1, 2):
                if not (
                    (
                        (f.edge[i].endpts[0] == f.vertex[i])
                        and (f.edge[i].endpts[1] == f.vertex[(i + 1) % 3])
                    )
                    or (
                        (f.edge[i].endpts[1] == f.vertex[i])
                        and (f.edge[i].endpts[0] == f.vertex[(i + 1) % 3])
                    )
                ):
                    ...
                    for j in (0, 1, 2):
                        ...
                        if (
                            (f.edge[j].endpts[0] == f.vertex[i])
                            and (f.edge[j].endpts[1] == f.vertex[(i + 1) % 3])
                        ) or (
                            (f.edge[j].endpts[1] == f.vertex[i])
                            and (f.edge[j].endpts[0] == f.vertex[(i + 1) % 3])
                        ):
    ...
    def DoubleTriangle(self):
        ...
        while Vertex.Collinear(
            self.vertices[v0 % nv],
            self.vertices[(v0 + 1) % nv],
            self.vertices[(v0 + 2) % nv],
        ):
            v0 = (v0 + 1) % nv
            if v0 == 0:
        ...
        self.faces.append(
            Face(
                vertex=[self.vertices[v0], self.vertices[v1], self.vertices[v2]]
            )
        )
        f0 = self.faces[-1]
        ...
        self.faces.append(
            Face(
                vertex=[self.vertices[v2], self.vertices[v1], self.vertices[v0]]
            )
        )
        f1 = self.faces[-1]
