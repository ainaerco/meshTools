...
class tubeDeformer(OpenMayaMPx.MPxNode):
    ...
    def compute(self, plug, dataBlock):
        if plug == tubeDeformer.outputMesh:
            ...
            MayaTube(
                curveFn=curve_fn,
                scale=profileScaleValue,
                profileFn=profile_fn,
                parent=newOutputData,
                scale_corners=1 - scaleCornersValue,
                segments=segmentsValue,
                cylinder_segments=circleSegmentsValue,
                fix=fixValue,
                rotate=profileRotateValue,
                taper=taperValue,
                twist=twistValue,
                cap=capValue,
                even=evenDistributeValue,
                growth=growthValue,
...
def nodeInitializer():
    ...
    tubeDeformer.profileScale = nAttr.create(
        "profileScale", "psc", OpenMaya.MFnNumericData.kDouble, 1.0
    )
    ...
    tubeDeformer.profileRotate = nAttr.create(
        "profileRotate", "prot", OpenMaya.MFnNumericData.kDouble, 0.0
    )
    nAttr.setKeyable(True)
    nAttr.setStorable(True)
    nAttr.setSoftMin(0)
    ...
    tubeDeformer.twist = nAttr.create(
        "profileTwist", "twist", OpenMaya.MFnNumericData.kDouble, 0.0
    )
    nAttr.setKeyable(True)
    nAttr.setStorable(True)
    nAttr.setSoftMin(0)
    ...
    tubeDeformer.taper = nAttr.create(
        "profileTaper", "taper", OpenMaya.MFnNumericData.kDouble, 0.0
    )
    nAttr.setKeyable(True)
    nAttr.setStorable(True)
    nAttr.setSoftMin(0)
    ...
    tubeDeformer.growth = nAttr.create(
        "growth", "g", OpenMaya.MFnNumericData.kDouble, 1.0
    )
    nAttr.setKeyable(True)
    nAttr.setStorable(True)
    nAttr.setMin(0)
    ...
    tubeDeformer.segments = nAttr.create(
        "segments", "segs", OpenMaya.MFnNumericData.kInt, 2
    )
    nAttr.setKeyable(True)
    nAttr.setStorable(True)
    nAttr.setMin(3)
    ...
    tubeDeformer.circleSegments = nAttr.create(
        "circleSegments", "circlesegs", OpenMaya.MFnNumericData.kInt, 8
    )
    nAttr.setKeyable(True)
    nAttr.setStorable(True)
    nAttr.setMin(1)
    ...
    tubeDeformer.fixType = nAttr.create(
        "fixType", "fix", OpenMaya.MFnNumericData.kInt, 0
    )
    nAttr.setMin(0)
    nAttr.setMax(3)
    ...
    tubeDeformer.outputMesh = typedAttr.create(
        "outputMesh", "out", OpenMaya.MFnData.kMesh
    )
    # add attribute
    ...
    tubeDeformer.attributeAffects(
        tubeDeformer.profileScale, tubeDeformer.outputMesh
    )
    tubeDeformer.attributeAffects(
        tubeDeformer.profileRotate, tubeDeformer.outputMesh
    )
    ...
    tubeDeformer.attributeAffects(
        tubeDeformer.segments, tubeDeformer.outputMesh
    )
    tubeDeformer.attributeAffects(
        tubeDeformer.circleSegments, tubeDeformer.outputMesh
    )
    ...
    tubeDeformer.attributeAffects(
        tubeDeformer.scaleCorners, tubeDeformer.outputMesh
    )
    tubeDeformer.attributeAffects(
        tubeDeformer.evenDistribute, tubeDeformer.outputMesh
    )
...
def initializePlugin(mobject):
    ...
    try:
        mplugin.registerNode(
            kPluginNodeTypeName, tubeDeformerId, nodeCreator, nodeInitializer
        )
    except Exception:
