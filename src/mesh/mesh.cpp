...
float
Vert::dot(const Vert& other)
{
	return v.dot(other.v);
}

Vector
Vert::cross(const Vert& other)
{
...
Face::computeNormal()
{
	...
	for(size_t i=0;i<vsize;i++)
	{
		if(i==vsize-1){normal += verts[i]->cross(*verts[0]);}
		else{normal += verts[i]->cross(*verts[i+1]);}
	}
	normal = normal.normalize();
	return normal;
}
