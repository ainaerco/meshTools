...
void swap(float first[3], float second[3])
{
     std::swap(first, second);
}
void
Bbox::obbFromPointSet(std::vector<Vector> pointset)
{
	Vector means;
	...
	for(int x=0;x<3;x++)
		for(int y=0;y<3;y++)
		{
			...
			{
				m[x][y]+=(means[x]-pointset[i][x])*(means[y]-pointset[i][y]);
			}
			m[x][y]/=pointset_size;
		}
	float a,b,c,d;
	a = -1;
	b = m[0][0]+m[1][1]+m[2][2];
	c = -m[0][0]*m[1][1]-m[0][0]*m[2][2]-m[1][1]*m[2][2]+m[0][1]*m[1][0]+m[1][2]*m[2][1]+m[0][2]*m[2][0];
	d = m[0][0]*m[1][1]*m[2][2]-m[0][1]*m[1][0]*m[2][2]-m[1][2]*m[2][1]*m[0][0]-m[0][2]*m[2][0]*m[1][1]
	...
	for (size_t i = 0; i < pointset_size; i++)
	{
		...
		const Vector& v = Vector(axis[0].dot(pointset[i]), axis[1].dot(pointset[i]), axis[2].dot(pointset[i]));
		if (i == 0)
		{
			max = v;
			min = v;
