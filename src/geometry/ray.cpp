...
bool Ray::pointPlaneSide(Vector point) {
    return direction.dot(point - origin) >= 0;
}

float Ray::pointDistance(Vector point) { return direction.dot(origin - point); }

Vector Ray::pointProjection(Vector point) {
    Vector v = point - origin;
    return point - direction * (v.dot(direction));
}

Vector Ray::triangleRayHit(Vector triangle[3]) {
...
Vector Ray::segmentPlaneHit(Vector segment0, Vector segment1) {
